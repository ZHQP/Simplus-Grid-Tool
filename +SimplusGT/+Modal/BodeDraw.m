function BodeDraw(ApparatusSel, AxisSel, GminSS, ApparatusType, ApparatusBus, N_Bus, ApparatusInputStr, ApparatusOutputStr, SysType)
...
for i=1:AxisNum
    ...
    elseif SysType == 2
        if AxisSel(i)==1 %dc axis;
            pinBias=0;
            poutBias=0;
            title='Admittance Bode Diagram: dc';
        end
...
for k = 1:N_Bus
    ...
    if ApparatusType{AppIndex} <= 89 || (ApparatusType{AppIndex} >=1010 && ApparatusType{AppIndex} <= 1089)% AC or DC apparatuses
